...
void Context::UpdateCamera(float delta) {
    ...
    if (camera.target.x < this->cam_target_destination.x - this->cam_target_speed.x) {
        camera.target.x += this->cam_target_speed.x;
    } else if (camera.target.x > this->cam_target_destination.x + this->cam_target_speed.x) {
        camera.target.x -= this->cam_target_speed.x;
    }

    if (camera.target.y < this->cam_target_destination.y - this->cam_target_speed.y) {
        camera.target.y += this->cam_target_speed.y;
    } else if (camera.target.y > this->cam_target_destination.y + this->cam_target_speed.y) {
        camera.target.y -= this->cam_target_speed.y;
    }

    if (camera.target.z < this->cam_target_destination.z - this->cam_target_speed.z) {
        camera.target.z += this->cam_target_speed.z;
    } else if (camera.target.z > this->cam_target_destination.z + this->cam_target_speed.z) {
        camera.target.z -= this->cam_target_speed.z;
    }
