...
int main(void) {
    ...
    while(!WindowShouldClose()) {
        ...
        if(IsKeyDown(KEY_S)) camera.position = (Vector3) {camera.position.x - (camera_forward.x * move_speed), camera.position.y - (camera_forward.y * move_speed), camera.position.z - (camera_forward.z * move_speed)};
